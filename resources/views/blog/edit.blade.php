...
<div class="container m-auto text-center pb-6 pt-10">
    <form action="/blog/{{$post->slug}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="text" name="title" value="{{$post->title}}" class="w-full text-3xl rounded-lg shadow-lg border-b">
        <textarea name="description" class="w-full text-3xl rounded-lg shadow-lg border-b mt-5">{{$post->description}}</textarea>
...
<div class="container sm:grid  mx-auto p-5 border-b border-gray-300">
    <button type="submit" class="bg-green-900 text-gray-300 py-4 px-5 rounded-lg font-bold uppercase text-l place-self-center">Submit Post</button>
</div>
    </form>
